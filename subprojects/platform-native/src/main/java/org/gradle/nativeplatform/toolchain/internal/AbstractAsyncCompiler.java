...
public abstract class AbstractAsyncCompiler<T extends BinaryToolSpec> extends AbstractCompiler<T> {
    ...
    public AbstractAsyncCompiler(BuildOperationExecutor buildOperationExecutor, CommandLineToolInvocationWorker commandLineToolInvocationWorker, CommandLineToolContext invocationContext, ArgsTransformer<T> argsTransformer, boolean useCommandFile, ExecutorFactory executorFactory) {
    ...
    public WorkResult execute(final T spec) {
        ListeningExecutorService executorService = MoreExecutors.listeningDecorator(executorFactory.create(this.getClass().getSimpleName(), 1));
        final Action<BuildOperationQueue<CommandLineToolInvocation>> invocationAction = newInvocationAction(spec);

        try {
            ListenableFuture<WorkResult> futureWorkResult = executorService.submit(new Callable<WorkResult>() {
                @Override
